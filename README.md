...
- **历史总览与回放模式**: 底部 dock 提供全库/时间窗包络与 brush；与 **Live/Review**（选区是否跟尾）及 **会话实时/离线库**（SQLite 数据源）两套模式正交组合，详见下文「数据管理 → 历史总览与时间/数据源模式」。
...
底部 dock **「历史总览」** 与 **阵列图**、**阵列 RGB 热力图** 共用两套 **正交** 的状态，组合后决定「读哪份库、查多长时间、选区是否跟尾」。实现上分别对应 `SelectionState`（时间选区 + Live/Review）与 `HistoryDataProvider::HistorySourceMode`（只读 SQLite 来自会话还是外部文件）。

##### 1. 时间选区模式（`SelectionState`，全局单例）


| 模式         | 含义                                                                                                                                                                                                                  |
| ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Live**   | 选区语义对齐 **当前数据尾部**：历史总览在 Live 定时刷新下会更新全局时间边界，并通过尾部对齐逻辑把选区右端对齐到最新 `max(timestamp)`（与 `SelectionState` 的写入带毫秒级去重，避免无意义重复刷新下游窗口）。阵列图/热力图在 **会话实时** 数据源下以实时快照为主，并对时间轴/缓冲施加 **最近约 1 小时** 的截取策略（与总览「1 小时窗」一致，减轻长跑内存与绘制压力）。 |
| **Review** | 选区由用户在总览 **brush**（拖动/拉伸时间框）明确指定；**不再** 被 Live 定时器强行拉回尾部。阵列图走 `renderReviewRange()`、热力图按 `timestamp` 过滤可见帧，从只读库按时间桶做 SQL 聚合。历史总览包络 SQL 的 **时间窗恢复为全库** `[min,max]`，便于查看早于最近 1 小时的内容。                                 |


**如何切换**：在总览上拖动选区；松手时若 **右缘已贴齐全局数据最大时间**（实现上对 `m_dataMaxMs` 留有 **2ms** 量级容差，避免像素反算误差）则进入 **Live**，否则进入 **Review**。也可随时点击 **「回到实时」**：强制 **Live** 并将选区对齐当前数据尾（即使当前为 Review）。详见 `HistoryOverviewWindow` 中 `commitRangeToSelectionState` / `syncLiveTailToSelectionState`。

##### 2. 历史 SQLite 数据源（`HistoryDataProvider::HistorySourceMode`）
...
当 `**HistorySourceMode::SessionRealtime` 且 `SelectionState::Live`** 时：

- **历史总览**：包络 SQL 与总览 X 轴仅覆盖 **最近约 1 小时**（`max − 3600s` 至 `max`，不足 1 小时则从全库 `min` 起）。状态栏在总览窗与全库不一致时会显示 **「全库: …  总览: …」**。
- **阵列图 / 热力图**：在同样条件下对 Live 路径做 **约 1 小时** 的数据截取（与总览策略一致）。

一旦切换到 **Review**（或数据源为 **OfflineExternal** 时的全库语义），总览包络恢复为 **全库时间跨度**（具体以 `plotEnvelopeStartMs()` / `plotEnvelopeEndMs()` 为准）。
...
1. 检查 sequence 是否有缺口（快速定位疑似丢帧区间）
