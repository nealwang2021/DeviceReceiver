...
/**
 * @brief 漏磁检测 gRPC 接收后端（mag_array.proto: MagArrayLeakageDetection）
 *
 * 支持两种工作模式：
 *   - Mock 模式（m_mockMode=true）：QTimer 驱动，本地生成含合成传感器数据的伪帧。
 *   - Real 模式（m_mockMode=false）：订阅服务端流式 RPC（StreamFrames），
 *     在独立的 std::thread 中阻塞读取 MagArrayFrame，通过 Qt::QueuedConnection 回传主线程。
 *
 * 线程安全：
